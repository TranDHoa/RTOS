...
Get App
Dưới đây là lộ trình 12 tuần được viết lại, bổ sung yêu cầu cụ thể cho từng tuần và kết quả đầu ra (deliverables) để bạn có thể tự đánh giá tiến độ học tập của mình.
...
Cài đặt thành công toolchain arm-none-eabi-gcc, OpenOCD, và trình giao tiếp debug (ST-Link/J-Link).

Viết chương trình C tối giản (không thư viện chuẩn) in "Hello World" qua semihosting, chạy trên target STM32.

Đọc và giải thích từng phần của file linker script (.ld): xác định vùng FLASH (RX), RAM (RW), và địa chỉ _estack.

Kết quả:

Build và debug thành công chương trình trên hardware thực.

File main.c, startup.s, linker.ld được kiểm soát hoàn toàn.

Giải thích được luồng: reset vector → Reset_Handler → main().

Tuần 2: Giao tiếp ngoại vi nguyên thủy
Yêu cầu:

Viết driver USART hoàn chỉnh chỉ bằng cách thao tác thanh ghi (không dùng HAL/LL).

Cấu hình RCC để bật xung cho USART, GPIO.

Gửi được chuỗi "UART works\r\n" ra console (PuTTY/Serial).

Kết quả:

Module uart.c và uart.h tự viết.

Hiểu và vẽ được sơ đồ Clock Tree cho USART được sử dụng.

Debug thành công việc giao tiếp ở tốc độ 115200 bps.

Tuần 3: Kiến trúc ARM Cortex-M & Assembly
Yêu cầu:

Viết lại file startup.s thủ công (thay vì dùng file mẫu) bao gồm:

Vector table với các entry: Reset_Handler, NMI_Handler, HardFault_Handler, SysTick_Handler,…

Định nghĩa stack pointer khởi tạo.

Reset_Handler gọi main().

Sử dụng assembly Thumb-2 để thực hiện phép tính đơn giản (cộng, nhân) và gọi hàm C.

Kết quả:

File startup.s tự viết, biên dịch và chạy đúng.

Hiểu rõ cách CPU lấy vector table, thiết lập SP, và nhảy vào main.

Chuẩn bị nền tảng để viết context switch bằng assembly ở tuần 9.

Phần 2: Cấu trúc Hệ điều hành & Đa nhiệm (Tuần 4–6)
Tuần 4: Phân quyền & Lời gọi hệ thống
Yêu cầu:

Phân biệt chế độ Thread mode (unprivileged) và Handler mode (privileged) qua CONTROL register.

Viết lệnh SVC (Supervisor Call) bằng assembly inline để chuyển từ unprivileged sang privileged, thực hiện một lệnh "đặc quyền" (ví dụ: ghi vào thanh ghi NVIC).

Kết quả:

Chương trình C có chứa __asm volatile("svc 0"); và handler SVC_Handler xử lý đúng.

Hiểu rõ cơ chế "system call" trong RTOS.

Tuần 5: Đa nhiệm hợp tác – Cooperative Multitasking
Yêu cầu:

Tạo hai task (Task A và Task B) chạy luân phiên bằng cách nhường quyền thủ công qua hàm yield().

Thực hiện context saving cơ bản: lưu các thanh ghi R4–R11 (theo AAPCS) vào stack của task.

Kết quả:

Code chạy được với hai task, mỗi task in ra một ký tự riêng, chạy xen kẽ.

Hiểu rõ điểm khác biệt giữa cooperative và preemptive scheduling.

Tuần 6: Trái tim của RTOS – Ngắt định thời SysTick
Yêu cầu:

Cấu hình SysTick timer tạo ngắt mỗi 1ms.

Trong handler SysTick, thực hiện tăng biến os_tick_count và đặt cờ để đánh dấu nhịp tim hệ thống.

Kết quả:

Đo và xác nhận chính xác tần số 1ms bằng oscilloscope hoặc toggle GPIO.

Hệ thống đã có nền tảng thời gian để phát triển bộ lập lịch.

Phần 3: Ứng dụng phần cứng & Bộ lập lịch (Tuần 7–9)
Tuần 7: Ứng dụng thực tế – Quét LED
Yêu cầu:

Cấu hình GPIO cho LED đơn và cụm LED 7 thanh.

Đưa thuật toán quét LED (multiplexing) vào SysTick_Handler mà không dùng delay().

LED đơn nhấp nháy với tần số xác định, LED 7 thanh hiển thị số thay đổi theo thời gian.

Kết quả:

Phần cứng hiển thị ổn định, không bị nhấp nháy do xung đột với vòng lặp chính.

Hiểu rõ cách RTOS quản lý tác vụ thời gian thực ngay cả trước khi có scheduler.

Tuần 8: Lập lịch ưu tiên – Preemptive Scheduling
Yêu cầu:

Kích hoạt ngắt PendSV (Pendable Service Call) để thực hiện context switch.

Trong SysTick_Handler, kích hoạt PendSV. Trong PendSV_Handler, thực hiện chuyển task.

Scheduler tự động "tước quyền" task đang chạy sau mỗi tick.

Kết quả:

Hai task chạy xen kẽ tự động, không cần yield() thủ công.

Hiểu rõ cơ chế: SysTick → PendSV → Context Switch.

Tuần 9: Tối ưu hóa Context Switch bằng Assembly
Yêu cầu:

Viết lại PendSV_Handler hoàn toàn bằng assembly.

Push/pop đầy đủ các thanh ghi R0–R3, R12, LR, PC, xPSR (hardware-stacked) và R4–R11 (software-stacked).

Sử dụng CPSID/CPSIE để đảm bảo atomic khi thay đổi con trỏ stack của task.

Kết quả:

Context switch hoạt động ổn định, tối ưu chu kỳ xung nhịp.

Sẵn sàng để mở rộng quản lý nhiều task.

Phần 4: Hoàn thiện & Chuẩn hóa (Tuần 10–12)
Tuần 10: Quản lý luồng cấp cao – Threads
Yêu cầu:

Định nghĩa cấu trúc TCB (Task Control Block) gồm: sp, priority, state, next (cho danh sách liên kết).

Xây dựng bộ lập lịch Round-Robin đơn giản duyệt danh sách TCB.

Hỗ trợ tạo task mới qua API os_create_task().

Kết quả:

Hệ thống chạy được 3–4 task độc lập, chuyển đổi theo vòng tròn.

Cấu trúc RTOS đã rõ ràng, có thể mở rộng.

Tuần 11: Trừu tượng hóa phần cứng với CMSIS
Yêu cầu:

Tích hợp CMSIS-Core vào project.

Refactor toàn bộ code thao tác thanh ghi (SysTick, NVIC, RCC) sang CMSIS API (ví dụ: NVIC_SetPriority, SysTick_Config).

Kết quả:

Code sạch hơn, ít phụ thuộc vào STM32 cụ thể.

Dễ dàng port sang các chip Cortex-M khác (LPC, nRF, …).

Tuần 12: Tổng duyệt & Mở rộng hướng IoT
Yêu cầu:

Kiểm tra toàn hệ thống hoạt động ổn định qua 24h liên tục.

Phân tích stack usage, độ trễ context switch bằng debugger.

Thiết kế một ứng dụng IoT nhỏ (ví dụ: đọc cảm biến, gửi qua radio, hiển thị LED) chạy trên RTOS đã tự viết.

Kết quả:

RTOS hoàn chỉnh, ổn định, có thể dùng trong các dự án nhúng thực tế.

Hiểu sâu cách RTOS giải quyết vấn đề trong các hệ thống phức tạp như node cảm biến không dây, tránh treo hệ thống khi xử lý nhiều luồng đồng thời.

Kết luận:
Lộ trình này không chỉ giúp bạn xây dựng một RTOS từ con số 0, mà còn đảm bảo bạn hiểu từng lớp từ hardware lên đến ứng dụng. Mỗi tuần đều có yêu cầu rõ ràng và kết quả kiểm chứng được, phù hợp để tự học hoặc hướng dẫn trong môi trường chuyên nghiệp.

Viết lại để tôi copy lên readmd trên github
Dưới đây là nội dung đã được định dạng lại để bạn copy trực tiếp vào file README.md trên GitHub. Sử dụng cú pháp Markdown với bảng biểu, checklist và cấu trúc rõ ràng.
